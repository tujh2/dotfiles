...
if [[ "$bat_status" == "Discharging" ]]; then
	if [[ "$bat_now"  -lt "20" ]]; then
		echo "  <span foreground=\"#FF0000\"> $bat_status</span> $bat_now %"
		...
		echo "  <span foreground=\"#00FF00\"> $bat_status</span> $bat_now %"
	fi
elif [[ "$bat_status" == "Charging" ]]; then
	echo " <span foreground=\"#00FF00\"> $bat_status $bat_now %</span>"
